...
agencies = pd.read_csv(
    "/mnt/c/Users/sdona/Documents/Duke/22Spring/701IDS/FinalProject/uds-2022-701-team-5/00_source_data/agencies.csv"
)
offense_type = pd.read_csv(
    "/mnt/c/Users/sdona/Documents/Duke/22Spring/701IDS/FinalProject/uds-2022-701-team-5/00_source_data/NIBRS_offense_type.csv"
)
incident_19 = pd.read_csv(
    "/mnt/c/Users/sdona/Documents/Duke/22Spring/701IDS/FinalProject/uds-2022-701-team-5/00_source_data/2019_NIBRS_incident.csv"
)
incident_20 = pd.read_csv(
    "/mnt/c/Users/sdona/Documents/Duke/22Spring/701IDS/FinalProject/uds-2022-701-team-5/00_source_data/2020_NIBRS_incident.csv"
)
offense_19 = pd.read_csv(
    "/mnt/c/Users/sdona/Documents/Duke/22Spring/701IDS/FinalProject/uds-2022-701-team-5/00_source_data/2019_NIBRS_offense.csv"
)
offense_20 = pd.read_csv(
    "/mnt/c/Users/sdona/Documents/Duke/22Spring/701IDS/FinalProject/uds-2022-701-team-5/00_source_data/2020_NIBRS_offense.csv"
)

# %%
# load intermediate data with fips codes, ori numbers and population
fips = pd.read_csv(
    "/mnt/c/Users/sdona/Documents/Duke/22Spring/701IDS/FinalProject/uds-2022-701-team-5/20_intermediate_files/ori.csv"
)
pop = pd.read_csv(
    "/mnt/c/Users/sdona/Documents/Duke/22Spring/701IDS/FinalProject/uds-2022-701-team-5/20_intermediate_files/population.csv"
)
...
merged_offenses = pd.merge(
    all_offenses,
    offense_type[["OFFENSE_TYPE_ID", "OFFENSE_CATEGORY_NAME"]],
    on="OFFENSE_TYPE_ID",
    how="left",
    validate="m:m",
    indicator=True,
...
merged_incidents = pd.merge(
    all_incidents,
    merged_ori[["AGENCY_ID", "COUNTY_NAME", "fips"]],
    on="AGENCY_ID",
    how="left",
    validate="m:1",
    indicator=True,
...
merged_crimes = pd.merge(
    merged_incidents.drop("_merge", axis=1),
    merged_offenses[["INCIDENT_ID", "OFFENSE_CATEGORY_NAME"]],
    on="INCIDENT_ID",
    how="left",
    indicator=True,
    validate="m:m",
...
final_crimes = (
    merged_crimes.groupby(["month", "DATA_YEAR", "fips", "OFFENSE_CATEGORY_NAME"])
    .agg({"crime_count": "sum"})
    .reset_index()
)
...
final_crimes.loc[
    final_crimes["OFFENSE_CATEGORY_NAME"].isin(violent_crimes), "violent_crime"
] = 1

# %%
# aggregate crimes by month, year, fips code, and whether it is a violent crime
final_crimes.groupby(["month", "DATA_YEAR", "fips", "violent_crime"]).agg(
    {"crime_count": "sum"}
).reset_index()
...
merged_pop = pd.merge(
    final_crimes,
    pop.drop(["year", "fips_state_county_code"], axis=1),
    on=["fips", "month", "DATA_YEAR"],
    how="inner",
)
